# rotation_matrix_origin_at_entrypoint
import numpy as np

def calculate_orientation(theta1, theta2, theta3, camera_height=60):
    ...
    # Vectors in local coordinates, assuming local Z is towards the wire (z-axis)
    # and considering the cameras' height at z = 60 mm
    ...
    # Transform local vectors to the global coordinate system
    v1_global = R1 @ v1_local
    v2_global = R2 @ v2_local
    v3_global = R3 @ v3_local

    # Adjusting for the camera height
    v1_global[2] -= camera_height
    v2_global[2] -= camera_height
    v3_global[2] -= camera_height
