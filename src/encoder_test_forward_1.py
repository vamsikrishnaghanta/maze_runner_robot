...
try:
    while True:
        ...
        # Define a synchronization PID controller right
        speed_error_right = current_pulse_count11 - current_pulse_count21
        speed_integral_right += speed_error_right
        speed_derivative_right = speed_error_right - previous_speed_error_right
        previous_speed_error_right = speed_error_right

        # Define a synchronization PID controller left
        speed_error_left = current_pulse_count21 - current_pulse_count11
        speed_integral_left += speed_error_left
        speed_derivative_left = speed_error_left - previous_speed_error_left
        previous_speed_error_left = speed_error_left

        # Calculate the control signal for synchronization
        pid_speed_control_signal_right = kp * speed_error_right + ki * speed_integral_right + kd * speed_derivative_right
        pid_speed_control_signal_left = kp * speed_error_left + ki * speed_integral_left + kd * speed_derivative_left

        print(f'pid_right:{pid_speed_control_signal_right}, pid_left:{pid_speed_control_signal_left}')
        ...
        motor_speed1 = max(0, min(40, motor_speed1))
        motor_speed2 = max(0, min(40, motor_speed2))
